...
> 699 tests, tamper-proof audit trail, 2,260 req/s load tested
...
1. **Works first.** Every commit runs the full test suite (699 tests). The demo path — upload four CSVs, get a dashboard, audit trail, and review queue — works without a single external API key.
...
2. Run the full test suite (699 tests)
...
POST /api/fetch-razorpay            — Fetch live settlements from Razorpay API
